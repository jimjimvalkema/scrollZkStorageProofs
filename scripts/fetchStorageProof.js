...
import { getHashPathFromProof, verifyHashPath } from "./decodeScrollProof"
...
export async function getProof({ provider, blockNumber, contractAddress, storageKey }) {
...
export async function getProofOfMapping({ provider, blockNumber, contractAddress, slot, key, keyType }) {
  const storageKey = hashStorageKeyMapping({ key, keyType, slot })
  const proof = await getProof({ provider, blockNumber, contractAddress, storageKey })
  return proof
...
async function main() {
  ...
  const parser = new ArgumentParser({
    ...
    usage: `\n mapping mode (gets weth balance of jimjim): ${mappingUsageExample}\n normal slot (gets weth name): ${normalSlotExample}`
  });

  // optional
  parser.add_argument('-p', '--provider', { help: 'Provider url. Default uis mainnet. ex: mainnet: --provider=https://scroll.drpc.org or sepolia: --provider=https://sepolia-rpc.scroll.io', required: false, default: "https://scroll.drpc.org" });
  ...
  if (args.mappingMode == true) {
    ...
    if (missingArgs.length) {
      ...
      const proof = await getProofOfMapping({ provider, blockNumber: args.blockNumber, contractAddress: args.contractAddress, slot: args.slot, key: args.key, keyType: args.keyType })
      console.log(proof)
      process.exit(0);
    ...
    if (extraArgs.length) {
      ...
      const proof = await getProof({ provider, blockNumber: args.blockNumber, contractAddress: args.contractAddress, storageKey: args.slot })
      console.log(proof)
      process.exit(0);
    }
  }
